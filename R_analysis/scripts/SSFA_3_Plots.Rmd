...
p_GP <- ggplot(GP_plot, aes_string(x = x_var_GP, y = "value", color = grp_colors)) +
        ...
        # The factor 3/4 ensures that the box have the same width than for the sheep dataset, 
        # which has 4 levels, while the GP dataset has only 3
        geom_boxplot(outlier.shape = NA, 
                     position = position_dodge(preserve = "single", width = 0.9*3/4),
                     width = 0.8*3/4) + 
  
        # Define jitter for points within boxplots
        # The factor 3/4 ensures that the box have the same width than for the sheep dataset, 
        # which has 4 levels, while the GP dataset has only 3
        geom_point(position = position_jitterdodge(jitter.width = 0.3*3/4,
                                                   dodge.width = 0.9*3/4,
                                                   seed = 123)) +
