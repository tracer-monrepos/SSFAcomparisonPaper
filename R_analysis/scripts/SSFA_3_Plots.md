...
p_GP <- ggplot(GP_plot, aes_string(x = x_var_GP, y = "value", color = grp_colors)) +
        ...
        geom_boxplot(outlier.shape = NA, width = 0.9*ratio_diet, size = 0.25, fatten = 3) + 
  
        # Define jitter for points within boxplots
        # The factor 'ratio_diet' ensures that the boxes have the same width 
        # as for the sheep dataset
        geom_point(mapping = aes_string(shape = grp_shapes, group = grp_colors), 
                   size = 0.8,
...
The only difference is that all surfaces have 0% NMP, so the code can be
simplified (no ‘shape’ aesthetics).
...
p_sheep <- ggplot(sheep_plot, aes_string(x = x_var_sheep, y = "value", color = grp_colors)) +
           geom_boxplot(outlier.shape = NA, width = 0.9, size = 0.25, fatten = 3) + 
           geom_point(size = 0.8,
...
p_GP_Smfc <- ggplot(GP_plot_Smfc_filt, 
...
```
    ...
    1 Clover  ConfoMap Smfc 12.28573
    2 Clover  ConfoMap Smfc 12.28573
    3  Grass  ConfoMap Smfc 44.73203
    4  Grass Toothfrax Smfc 89.99452
