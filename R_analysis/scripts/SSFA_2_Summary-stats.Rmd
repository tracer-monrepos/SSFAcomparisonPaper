...
## Filter out surfaces with too many non-measured points (NMP)
```{r}
nmp10 <- filter(all_data, NMP < 10)
data_nmp10 <- filter(all_data, Name %in% nmp10[["Name"]])

nmp5 <- filter(all_data, NMP < 5)
data_nmp5 <- filter(all_data, Name %in% nmp5[["Name"]])
```
